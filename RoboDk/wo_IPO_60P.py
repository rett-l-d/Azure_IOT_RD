...
class ThreadedClient(threading.Thread):
    ...
    #        self.sock1.setblocking(0)
            ...
            print("Error connecting to CNC Server. Start or reboot PC.")
            while not connected:
                try:
                    self.sock1.connect((self.server_address, self.port))
                    connected = True
    ...
    def run(self):
        print("Sucess: Connected to CNC Controller!")
        print("Start the decoder please.")
        while 1:
...
def send_data(sockp = socket):
    ...
    for i in to_plc:
       ...
       # data = struct.pack('f', i)
        #sockp.sendall(data)
    data = struct.pack('%sf' % len(data_arr), *data_arr)
    sockp.sendall(data)
...
#robot.setConnectionParams('192.168.1.99', 7000, 'C:/KukaShared', 'anonymous', '')
#robot.Connect()


#Connect to the robot using default connetion parameters
robot.setConnectionParams('10.42.13.97', 7000, 'C:/KukaShared', 'anonymous', '')
robot.Connect()
...
if status != ROBOTCOM_READY:
    ...
    connected = False
    #print("Error: ", socketerror)
    print("Connection not estabilished yet. Re-trying...")
    while not connected:
            robot.setConnectionParams('10.42.13.97', 7000, 'C:/KukaShared', 'anonymous', '')
            robot.Connect()
            status, status_msg = robot.ConnectedState()
...
print("Sucess: Connected to Robot Controller")
...
robot_name= ''
robot_name = RDK.ItemList(filter = ITEM_TYPE_ROBOT, list_names=True)
print(robot_name)
#if robot_name[0].find('KUKA') > - 1:
...
#robot.setJoints([-1.95,-91.38,100.58,0,80.80,-1.95])
...
#robot.setZoneData(0) # Set the rounding parameter (Also known as: CNT, APO/C_DIS, ZoneData, Blending radius, cornering, ...)
#robot.setRounding(1)
